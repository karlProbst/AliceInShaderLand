...
func _physics_process(delta):
	if target:
		...
		#####
		print(raycastr.is_colliding())
		if raycastr.is_colliding():
			print("djsaljkfas")
			move_direction = (flat_direction + Vector3(-4, 0, 0)).normalized()
			spd=max_speed
		elif raycastl.is_colliding():
			move_direction = (flat_direction + Vector3(4, 0, 0)).normalized()
			spd=max_speed
		...
		velocity.x = move_direction.x * current_speed * spd
		velocity.z = move_direction.z * current_speed * spd
